...
class lc3548 {
    private func can_partition(_ grid: [[Int]]) -> Bool {
        ...
        // case1 1 row
        if rows == 1 {
            r_vals[grid[0][1], default: 0] += 1
            r_vals[grid[0][cols-1], default: 0] += 1

            for j in 1..<cols {
                total_r_sum += total_col_sum[j]
            }

            if total_l_sum == total_r_sum || r_vals.keys.contains(total_r_sum-total_l_sum) {
                return true
            }

            l_vals[grid[0][0], default: 0] += 1
            for j in 1..<cols-1 {
                l_vals[grid[0][j], default: 0] += 1
                if j-1>0 {
                    let k = grid[0][j-1]
                    l_vals[k, default: 0] -= 1
                    if l_vals[k, default: 0] == 0 {
                        l_vals.removeValue(forKey: k)
                    }
                }

                let k2 = grid[0][j]
                r_vals[k2, default: 0] -= 1
                if r_vals[k2, default: 0] == 0 {
                    r_vals.removeValue(forKey: k2)
                }

                if j+1 < cols-1 {
                    let k3 = grid[0][j+1]
                    r_vals[k3, default: 0] += 1
                }

                total_l_sum += total_col_sum[j]
                total_r_sum -= total_col_sum[j]
                if total_l_sum == total_r_sum
                    || (l_vals.keys.contains(total_l_sum - total_r_sum))
                    || (j < cols-2 && r_vals.keys.contains(total_r_sum - total_l_sum)) {
                    return true
                }
            }
        ...
        if total_r_sum == total_l_sum
            ...
            || (r_vals.keys.contains(total_r_sum - total_l_sum) && cols>2)
        {
            return true
        }
